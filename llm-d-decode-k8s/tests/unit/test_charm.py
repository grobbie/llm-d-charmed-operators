...
from charm import LlmdDecodeK8sCharm

@pytest.fixture
def harness():
    h = Harness(LlmdDecodeK8sCharm)
    h.begin()
    yield h
    h.cleanup()

def test_missing_model_id(harness):
    harness.set_can_connect("llm-d-decode", True)
...
def test_full_cluster_topology(mock_client, mock_is_ready, mock_get_endpoint, harness):
    ...
    # Setup KV relation
    kv_rel_id = harness.add_relation("kv-cache-manager", "kv-manager")
    harness.add_relation_unit(kv_rel_id, "kv-manager/0")
    harness.update_relation_data(kv_rel_id, "kv-manager/0", {"endpoint": "tcp://10.1.1.1:5557"})
    
    router_rel_id = harness.add_relation("decode-worker", "router")
    harness.add_relation_unit(router_rel_id, "router/0")
    ...
    # Check pebble plan
    plan = harness.get_container_pebble_plan("llm-d-decode")
    assert plan.services["llm-d-decode"] is not None
    assert plan.services["llm-d-decode"].command == "/opt/launch_vllm.sh"
    
    script_content = harness.model.unit.get_container("llm-d-decode").pull("/opt/launch_vllm.sh").read()
...
def test_pebble_connection_failure(harness):
    harness.set_can_connect("llm-d-decode", False)
    harness.update_config({})
