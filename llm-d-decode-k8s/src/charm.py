...
SERVICE_NAME = "llm-d-decode"


class LlmdDecodeK8sCharm(ops.CharmBase):
    """Juju Charm for managing the LLMD Decode Worker.
    
    This charm orchestrates the vLLM engine optimized for memory-bound autoregressive decoding.
    It receives KV cache fragments via the Nixl connector, rapidly synthesizes succeeding text generations,
    and reliably registers local token block allocations across the KV Cache Manager via ZeroMQ.
    """
    @property
    ...
    def __init__(self, framework: ops.Framework):
        ...
        self.container = self.unit.get_container("llm-d-decode")
        self.framework.observe(self.on.llm_d_decode_pebble_ready, self._on_pebble_ready)
        self.framework.observe(self.on.config_changed, self._on_config_changed)

        self.framework.observe(
            self.on.decode_worker_relation_joined, self._on_decode_relation_joined
        )
        self.framework.observe(
            self.on.kv_cache_manager_relation_changed, self._on_config_changed
        )
    ...
    def _on_decode_relation_joined(self, event):
    ...
    def _update_layer(self, event):
        ...
        self.container.add_layer("llm-d-decode", layer, combine=True)
